...
public class Main
{
	...
	public static void editReceptionist(int index)
	{
		...
		headingsDisplayer( "Receptionist", receptionistsListDetails, true );
		headingsDisplayer( "Receptionist", receptionistsListDetails, false );
	...
	public static void checkUpcomingAppointments()
	{
		...
		for (String[] appointmentListDetail : appointmentsList)
		{
			...
			if (docID.equals( appointmentListDoctorID ) && appointmentStatus.equals( "Pending" ))
			{
				if (!headingsShowed)
				{
					headingsDisplayer( "Appointment", appointmentListDetail, true );
				}
				headingsDisplayer( "Appointment", appointmentListDetail, false );
			}
		}
	}
	...
	public static void headingsDisplayer(String entity, String[] arrayOfEntity, boolean displayheadingsOnly)
	{
		switch (entity)
		{

			case "Doctor":
				processDoctorDisplay( arrayOfEntity, displayheadingsOnly );
				break;

			case "Appointment":
				processAppointmentDisplay( arrayOfEntity, displayheadingsOnly );
				break;

			case "Diagnosis":
				processDiagnosisDisplay( arrayOfEntity, displayheadingsOnly );
				break;

			case "Patient":
				processPatientDisplay( arrayOfEntity, displayheadingsOnly );
				break;

			case "Receptionist":
				processReceptionistDisplay( arrayOfEntity, displayheadingsOnly );
				break;

			case "Ward":
				processWardDisplay( arrayOfEntity, displayheadingsOnly );
				break;
		}
	}

	public static void processWardDisplay(String[] wardData, boolean displayWardHeadingsOnly)
	{
		if (displayWardHeadingsOnly)
		{
		...
		else
		{
			...
			String totalBeds = wardData[2];
			String occupiedBeds = wardData[3];
			String wardType = wardData[4];

			System.out.printf( "%-15s %-20s %-15s %-15s %-10s\n", wardId, wardName, totalBeds, occupiedBeds, wardType );
		}
	}

	public static void processReceptionistDisplay(String[] receptionistData, boolean displayReceptionistHeadingsOnly)
	{
		if (displayReceptionistHeadingsOnly)
		{
			System.out.printf( "%-15s %-20s %-15s %-15s\n", "Receptionist ID", "Receptionist Name", "Username", "Password" );
		}
		else
		{
			String recepId = receptionistData[0];
			String recepName = receptionistData[1];
			String username = receptionistData[2];
			String password = receptionistData[3];

			System.out.printf( "%-15s %-20s %-15s %-15s\n", recepId, recepName, username, password );
		}
	}

	public static void processPatientDisplay(String[] patientData, boolean displayPatientHeadingsOnly)
	{
		if (displayPatientHeadingsOnly)
		{
		...
		else
		{
			...
			String gender = patientData[2];
			String age = patientData[3];
	...
	public static void processDiagnosisDisplay(String[] diagnosisData, boolean displayDiagnosisHeadingsOnly)
	{
		if (displayDiagnosisHeadingsOnly)
		{
		...
		{
			String diagnosisId = diagnosisData[0];
			String patientIdDiagnosis = diagnosisData[1];
			String doctorIdDiagnosis = diagnosisData[2];
			String prescriptions = diagnosisData[3];
			String diagnosis = diagnosisData[4];
			String appointmentIdDiagnosis = diagnosisData[5];
	...
	public static void processAppointmentDisplay(String[] appointmentData, boolean displayAppointmentHeadingsOnly)
	{
		if (displayAppointmentHeadingsOnly)
		{
		...
		else
		{
			...
			String patientIdAppointment = appointmentData[1];
			String doctorIdAppointment = appointmentData[2];
			String appointmentTime = appointmentData[3];
			String appointmentDate = appointmentData[4];
			String appointmentStatus = appointmentData[5];
	...
	public static void processDoctorDisplay(String[] doctorData, boolean displayDoctorHeadingsOnly)
	{
		if (displayDoctorHeadingsOnly)
		{
		...
		else
		{
			...
			String startTime = doctorData[2];
			String endTime = doctorData[3];
			String ward = doctorData[4];
			String specialty = doctorData[5];
			// Count the number of appointments made by each doctor
			long appointmentsCount = countAppointmentsForDoctor( doctorId );
			System.out.printf( "%-10s %-20s %-15s %-15s %-20s %-20s %-15s\n", doctorId, doctorName, startTime,
